...
function queueSongs(bot, msg, videos) {
	if(videos.length > 0) {
			...
			request("http://api.soundcloud.com/resolve.json?url=" + videos[0] + "&client_id=71dfa98f05fa01cb3ded3265b9672aaf", function (error, response, body) {
				if(error) {
					bot.reply(error);
				  	...
				  	if(body.tracks) {
				  		bot.reply(msg, "More than 1 song was found. Please use !playlist to queue these songs.");
				  	}
				  	else {
						var message = "***#" + body.id + "***\nQueued **" + body.title + "**";
						message +=  " by **" + body.user.username + "** *(" + body.playback_count + " times played)* " + timeFormat(body.duration/1000);
						bot.sendMessage(boundedChannel, message);
						var sc_logo = fs.createReadStream("./lib/sc_logo.png");
						bot.sendFile(boundedChannel, sc_logo, "soundcloud_logo.png", function(err) {
							if(err.status == 403) {
								bot.sendMessage(boundedChannel, "**Powered by SoundCloud**");
							}	
						});
						if(!currentVideo) {
							play(bot, body);
					...
					bot.reply(msg, "Error: " + response.statusCode + " - " + response.statusMessage);
				}
				videos.shift();
		...
		if(videos[0].indexOf("youtube.com") > -1) {
		...
		ytdl.getInfo(requestUrl, function(err, info) {
			if(err) {
				...
				bot.sendMessage(boundedChannel, message, function(e, ytInfoMsg) {
					bot.deleteMessage(ytInfoMsg, {wait:300000})
				});
				if(!currentVideo) {
					play(bot, info);
				}
...
var commands = {
    ...
    "!playlist": {
    	...
    	process: function(bot, msg) {
	    		...
	    		request("http://api.soundcloud.com/resolve.json?url=" + playlist + "&client_id=71dfa98f05fa01cb3ded3265b9672aaf", function (error, response, body) {
					if(error) {
						...
						if(body.tracks) {
							...
							bot.reply(msg, "Playlist not found. Use !queue to queue a single song.");
						}
					}
					else {
						bot.reply(msg, "Error: " + response.statusCode + " - " + response.statusMessage);
    		...
    		if(playlist.indexOf("youtube.com") > -1) {
    ...
    "!link": {
    	...
    	process: function(bot, msg) {
    		...
    		if(currentVideo.loaderUrl) {
    			bot.sendMessage(msg.channel, currentVideo.loaderUrl);
    		}
    		else if (currentVideo.permalink_url) {
    			bot.sendMessage(msg.channel, currentVideo.permalink_url);
    		}
    	}
    },
    "!list": {
